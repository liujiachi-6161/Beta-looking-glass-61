...
let stars = [];
let collapse = false;
let expanse = false; // 炸开之后永久true，不再收回
let startTime;
let animationId;
...
// 视图切换【修复滚动bug：不再window.scrollTo，操作contentLayer内部滚动】
function switchView(view) {
    ...
    // 修复核心bug：重置contentLayer内部滚动，不是window
...
function askSpirit() {
    ...
    out.innerHTML = `<div style="color:var(--text2)">✨ 当前上下文：${spiritContext||"无"}<br><br>你的问题：${q}<br><br>⚠️ 静态本地版本，未接入后端大模型接口。<br>如需AI解读，需要部署后端提供 /api/chat 接口。</div>`;
    inp.value = '';
}
